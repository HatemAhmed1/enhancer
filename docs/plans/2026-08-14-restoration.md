...
- Inverse telecine reduces frame count by roughly one fifth, but Plan 1.5's segmenting sizes segments from `profile.frame_count`, which is the pre-filter count, and seeks with `-ss` in *input* time while decimation happens after that seek. A resumed segment computed from the pre-filter count would not align with what ffmpeg actually decoded. Rather than attempt a seek-aware fix, `render_resumable` now detects `decimate` in the active filter chain and forces the whole source to render as a single segment, forfeiting resumability for that job rather than risking a misaligned splice. `cmd_video` prints a one-line warning when this triggers. A true fix — deriving the post-filter output length before segmenting — is deferred; it would need either a first pass over the filtered stream to count frames, or a formula tied to the specific pulldown pattern detected.
